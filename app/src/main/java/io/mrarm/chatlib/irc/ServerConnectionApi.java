...
public abstract class ServerConnectionApi implements ChatApi {
    ...
    public Future<Void> subscribeChannelInfo(String channelName, ChannelInfoListener listener,
                                             ResponseCallback<Void> callback, ResponseErrorCallback errorCallback) {
        return SimpleRequestExecutor.run(new Callable<Void>() {
            @Override
            public Void call() throws Exception {
                ServerConnectionApi.this.getChannelData(channelName).subscribeInfo(listener);
                return null;
            }
        }, callback, errorCallback);
    }
    ...
    @Override
    public Future<Void> unsubscribeChannelInfo(String channelName, ChannelInfoListener listener, ResponseCallback<Void> callback, ResponseErrorCallback errorCallback) {
        return SimpleRequestExecutor.run(new Callable<Void>() {
            @Override
            public Void call() throws Exception {
                ServerConnectionApi.this.getChannelData(channelName).unsubscribeInfo(listener);
                return null;
            }
        }, callback, errorCallback);
    }
    ...
    public Future<Void> subscribeStatusMessages(StatusMessageListener listener, ResponseCallback<Void> callback,
        ...
        return SimpleRequestExecutor.run(new Callable<Void>() {
            @Override
            public Void call() throws Exception {
                ServerConnectionApi.this.getServerConnectionData().getServerStatusData().subscribeMessages(listener);
                return null;
            }
        }, callback, errorCallback);
    }
